...
def draw_frame(stdscr, title, sidebar_width=0, taskbar_height=0):
    ...
    stdscr.addstr(0, max(sidebar_width + 1, (w + sidebar_width)//2 - len(title_str)//2), title_str)
...
def explorer(stdscr):
    ...
    while True:
        ...
        if k in [ord('w'), curses.KEY_UP] and sel > 0: sel -= 1
            ...
            if name:
                with open(os.path.join(curr, name), 'w') as f: f.write("")
            ...
            stdscr.addstr(h-taskbar_height-3, content_start_x, f"'{items[sel]}' in Zwischenablage. Navigieren und [V] zum Einfügen.", curses.color_pair(6))
        elif k == ord('v') and clipboard: # Paste
            try:
                shutil.move(clipboard, os.path.join(curr, os.path.basename(clipboard)))
                clipboard = None
            except Exception as e:
                stdscr.addstr(h-taskbar_height-3, content_start_x, f"Fehler: {str(e)[:w-content_start_x-10]}", curses.color_pair(5))
...
def commands_view(stdscr):
    ...
    while True:
        ...
        if k in [ord('w'), curses.KEY_UP]:
            ...
            print("\nBeliebige Taste zum Fortfahren..."); curses.getch()
            stdscr.clear(); apply_colors(); curses.curs_set(0)
        elif k == ord('q'): break
...
def hacking_tools(stdscr):
    ...
    while True:
        ...
        for i, p in enumerate(HACK_PAGES):
            ...
            stdscr.addstr(3, content_start_x + i*12, f" {p['n']} ", attr)
        ...
        k = stdscr.getch()
        if k in [ord('a'), curses.KEY_LEFT] and sel_p > 0: sel_p -= 1; sel_t = 0
        elif k in [ord('d'), curses.KEY_RIGHT] and sel_p < len(HACK_PAGES)-1: sel_p += 1; sel_t = 0
        elif k in [ord('w'), curses.KEY_UP] and sel_t > 0: sel_t -= 1
        elif k in [ord('s'), curses.KEY_DOWN] and sel_t < len(tools)-1: sel_t += 1
        elif k in [10, 13]:
            curses.endwin()
